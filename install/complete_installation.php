...
################################################################################
	...
	if($passed_step == 5){ 
		...
		$site_email			 = isset($_SESSION['site_email']) ? prepare_input($_SESSION['site_email']) : '';
		$site_owner			 = isset($_SESSION['site_owner']) ? prepare_input($_SESSION['site_owner']) : '';
		$site_owner_url      = isset($_SESSION['site_owner_url']) ? stripslashes($_SESSION['site_owner_url']) : '';
		$install_type		 = isset($_SESSION['install_type']) ? $_SESSION['install_type'] : 'create';

		$licence_key		 = isset($_SESSION['licence']) ? stripslashes($_SESSION['licence']) : '';
		$site_server 	 	 = $_SERVER['HTTP_HOST'];
		...
		if(empty($site_email)) $error_mg[] = lang_key('alert_http_server_empty');	 
					...
					if(EI_CHECK_DB_MINIMUM_VERSION && (version_compare($db->GetVersion(), EI_DB_MINIMUM_VERSION, '<'))){
						...
						if($sql_dump != ''){
							if(false == ($db_error = apphp_db_install($sql_dump_file))){
								...
								$config_file = str_replace('<SITE_EMAIL>', $site_email, $config_file);
								$config_file = str_replace('<SITE_COPY_OWNER>', $site_owner, $config_file);
								$config_file = str_replace('<SITE_COPY_OWNER_LINK>', $site_owner_url, $config_file); 
